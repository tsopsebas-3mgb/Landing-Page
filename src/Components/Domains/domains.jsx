...
export const Domains = ({indexx,setIndexC,name,logo,more,activities})=>{
    const [fade,setFade]=useState(null);
    const [isMob,setIsMob]=useState(window.innerWidth<770);
    useEffect(() => {
        const resize=()=>{
            ...
            <div className={styles.keys}>
                ...
                <div className={styles.navs}>
                    {activities.map((obj, index) => (
                        <div  className={`${styles.nav} ${index === indexx ? styles.red : ""}`}></div>
                    ))}
                </div>
            </div>
        </div>
        ...
        <div className={styles.bop}>
            {activities.map((obj, index) => (
                <div className={`${styles.container} ${fade? styles.in:styles.out}`}>
                    ...
                    <div className={styles.count}>
                        <div className={styles.subCount}>{obj.description}</div>
                    </div>
